...
public class GameManagement : MonoBehaviour
{
    ...
    private float native_width = 750.0f;

    private float native_height = 1334.0f;
    ...
    private int numBadButtons = 10;

    private int numGoodButtons = 4;
    ...
    private int addMoreTimeGoal = 10;

    //Variable for how much time should be added;
    private float additionalTime = 4.0f;
    ...
    private void Update()
    {
        ...
        if (Input.touchCount > 0 && noMiss)
        { //Make sure there is a tap on the screen
        ...
        else if (Input.GetMouseButtonDown(0) && noMiss)
        { //If you tap and you can not miss check to see if you missed
            CheckHit(Input.mousePosition);
        }
    }

    //Method used to determine the pitch of the background music
    private void PitchChecker()
    {
        if (guiCountDown > 0)
        {
    ...
    private void CheckHit(Vector3 tempPos)
    {
        ...
        { //Returns true if the raycast intersected a collider and sets it to "hit"
    ...
    private void GenerateButtons(int gameMode)
    {
        if (gameMode == 0)
        {
    ...
    private void BeginCountDown(float time)
    {
        gameOver = false;
    ...
    private void TapStartCountDown()
    {
        guiCountDown--;
        if (guiCountDown == 0)
        {
    ...
    public void IncreaseScore()
    {
        ...
        if (!showMenu && !showReplayScreen)
        {
            ...
            if (gameMode == 0)
            {
                ...
                if (streak % addMoreTimeGoal == 0)
                { //Only increase the timer if it is the challenge mode
    ...
    {
        PlayArcadeIntegration.Instance.SubmitScore(score, "thar she blows");
        PlayArcadeIntegration.Instance.GetHighScores();
        gameOver = true;
        CancelInvoke("CountDown");
        RemoveButtons();
        ShowReplayScreen();
